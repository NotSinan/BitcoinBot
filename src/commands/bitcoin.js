...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const url = await createChartUrl('https://api.coincap.io/v2/assets/bitcoin/history?interval=d1')
        ...
        await interaction.editReply({content:url, components: [row]});
         ...
         collector.on('collect', async i => {
          let chartUrl;
          if (i.customId === '5m') {
              chartUrl = await createChartUrl('https://api.coincap.io/v2/assets/bitcoin/history?interval=m5')
              await i.update({ content: chartUrl, components: [row] });
          } else if (i.customId === '30m') {
              chartUrl = await createChartUrl('https://api.coincap.io/v2/assets/bitcoin/history?interval=m30')
              await i.update({ content: chartUrl, components: [row] });
          } else if (i.customId === '1h') {
              chartUrl = await createChartUrl('https://api.coincap.io/v2/assets/bitcoin/history?interval=h1')
              await i.update({ content: chartUrl, components: [row] });
          } else if (i.customId === '1d') {
              chartUrl = await createChartUrl('https://api.coincap.io/v2/assets/bitcoin/history?interval=d1')
              await i.update({ content: chartUrl, components: [row] });
          }
      });

      collector.on('end', () => {
        interaction.channel.send('Sorry, the time has run out for this interaction. Please try again if you would like to use this feature.')
      });
